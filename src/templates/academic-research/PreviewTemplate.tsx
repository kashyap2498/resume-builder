// =============================================================================
// Academic Research - Preview Template (HTML/CSS)
// =============================================================================
// Academic CV format with publications prominent, traditional layout.

import React from 'react';
import type { TemplateProps } from '@/types/template';
import type { SectionConfig } from '@/types/resume';
import { EntryBlock, ContactLine, SectionHeading, SkillsBlock, CustomContentBlock } from '../shared';
import { formatDateRange } from '../shared/DateRange';

const AcademicResearchPreview: React.FC<TemplateProps> = ({ resume }) => {
  ...
  const sectionTitleStyle: React.CSSProperties = {
    fontSize: `${font.sizes.sectionHeader}px`, fontFamily: font.headerFamily, color: accentColor, fontWeight: 700,
    margin: 0, marginBottom: '4px', lineHeight: font.lineHeight, textTransform: 'uppercase', letterSpacing: '1px',
    borderBottom: `1px solid ${accentColor}`, paddingBottom: '3px',
  };
  ...
  const renderSection = (section: SectionConfig) => {
    switch (section.type) {
      case 'contact':
        return (
          <div key={section.id} style={{ marginBottom: `${layout.sectionSpacing}px`, textAlign: 'center', borderBottom: `2px solid ${accentColor}`, paddingBottom: '8px' }}>
            ...
            {data.contact.title && <div style={{ fontSize: `${font.sizes.title}px`, fontFamily: font.family, color: colors.lightText, marginTop: '2px' }}>{data.contact.title}</div>}
            <div style={{ marginTop: '6px' }}><ContactLine contact={data.contact} font={font} colors={colors} layout="line" separator="\u2022" /></div>
          </div>
        );

      case 'summary':
        if (!data.summary.text) return null;
        return (<div key={section.id} style={{ marginBottom: `${layout.sectionSpacing}px` }}><h2 style={sectionTitleStyle}>Research Interests</h2><p style={{ fontSize: `${font.sizes.normal}px`, fontFamily: font.family, color: colors.text, lineHeight: font.lineHeight, margin: '4px 0 0 0' }}>{data.summary.text}</p></div>);
      ...
      case 'publications':
        ...
        return (
          <div key={section.id} style={{ marginBottom: `${layout.sectionSpacing}px` }}>
            ...
            {data.publications.map((entry, idx) => (
              <div key={entry.id} style={{ marginBottom: `${layout.itemSpacing}px`, paddingLeft: '24px', textIndent: '-24px' }}>
                <span style={{ fontSize: `${font.sizes.normal}px`, fontFamily: font.family, color: colors.text, lineHeight: font.lineHeight }}>
                  [{idx + 1}] {entry.title}. <em>{entry.publisher}</em>, {entry.date}.
      ...
      case 'skills':
        ...
        return (<div key={section.id} style={{ marginBottom: `${layout.sectionSpacing}px` }}><h2 style={sectionTitleStyle}>{section.title}</h2><SkillsBlock skills={data.skills} layout={data.skillsLayout} mode={data.skillsMode} font={font} colors={colors} categoryWeight={700} categoryColor={colors.text} /></div>);

      case 'projects':
        if (data.projects.length === 0) return null;
        return (<div key={section.id} style={{ marginBottom: `${layout.sectionSpacing}px` }}><h2 style={sectionTitleStyle}>Research Projects</h2>{data.projects.map((entry) => (<EntryBlock key={entry.id} title={entry.name} subtitle={entry.technologies.length > 0 ? entry.technologies.join(', ') : undefined} dateRange={formatDateRange(entry.startDate, entry.endDate)} description={entry.description} highlights={entry.highlights} font={font} colors={colors} spacing={layout.itemSpacing} />))}</div>);
...
export default AcademicResearchPreview;
