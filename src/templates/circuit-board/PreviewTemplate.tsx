// =============================================================================
// Circuit Board - Preview Template (HTML/CSS)
// =============================================================================
// Two-column for electrical/hardware engineers. Left sidebar with skills
// and certifications, main with experience. Technical green/dark palette.
...
const CircuitBoardPreview: React.FC<TemplateProps> = ({ resume }) => {
    ...
    switch (section.type) {
          ...
          <div key={section.id} style={{ marginBottom: `${layout.sectionSpacing}px` }}>
            ...
            {data.certifications.map((entry) => (
              <div key={entry.id} style={{ marginBottom: '6px' }}>
                <div style={{ fontWeight: 600, fontSize: `${font.sizes.small}px`, fontFamily: font.family, color: colors.text }}>{entry.name}</div>
                <div style={{ fontSize: `${font.sizes.small - 1}px`, fontFamily: font.family, color: colors.lightText }}>{entry.issuer}</div>
        ...
        return (
          <div key={section.id} style={{ marginBottom: `${layout.sectionSpacing}px` }}>
            ...
            {data.languages.map((lang) => (
              <div key={lang.id} style={{ fontSize: `${font.sizes.small}px`, fontFamily: font.family, color: colors.text, marginBottom: '3px' }}>
      ...
      case 'hobbies':
        ...
        return (
          <div key={section.id} style={{ marginBottom: `${layout.sectionSpacing}px` }}>
            <h2 style={sidebarTitleStyle}>{section.title}</h2>
            <div style={{ fontSize: `${font.sizes.small}px`, fontFamily: font.family, color: colors.text }}>
  ...
  return (
    <div style={{ width: '100%', maxWidth: '816px', margin: '0 auto', backgroundColor: colors.background, fontFamily: font.family, color: colors.text, lineHeight: font.lineHeight }}>
      ...
      <div style={{ padding: `${layout.margins.top}px ${layout.margins.right}px 0 ${layout.margins.left}px`, borderBottom: `3px solid ${accentColor}`, marginBottom: '0' }}>
        <h1 style={{ fontSize: `${font.sizes.name}px`, fontFamily: '"Courier New", Courier, monospace', color: colors.text, fontWeight: 700, margin: 0, lineHeight: font.lineHeight }}>
          {data.contact.firstName} {data.contact.lastName}
        </h1>
        {data.contact.title && (
          <div style={{ fontSize: `${font.sizes.title}px`, fontFamily: font.family, color: accentColor, marginTop: '2px', marginBottom: '8px' }}>
            {data.contact.title}
          </div>
        )}
      ...
      <div style={{ display: 'flex' }}>
        <div style={{ width: '32%', padding: `${layout.sectionSpacing}px ${layout.margins.left * 0.6}px ${layout.margins.bottom}px ${layout.margins.left}px`, borderRight: `1px solid ${colors.divider}` }}>
          {sidebarSections.map(renderSidebarSection)}
        </div>
        <div style={{ width: '68%', padding: `${layout.sectionSpacing}px ${layout.margins.right}px ${layout.margins.bottom}px ${layout.margins.left * 0.8}px` }}>
...
export default CircuitBoardPreview;
