// =============================================================================
// Creative Portfolio - PDF Template (@react-pdf/renderer)
// =============================================================================
...
const CreativePortfolioPdf: React.FC<TemplateProps> = ({ resume }) => {
  ...
  const styles = StyleSheet.create({
    ...
    name: { fontSize: font.sizes.name + 4, fontFamily: headerFont, fontWeight: 700 as const, color: '#ffffff', marginBottom: 8, lineHeight: 1.2 },
    jobTitle: { fontSize: font.sizes.title, fontFamily: bodyFont, fontWeight: 700 as const, color: accentColor, marginBottom: 8 },
    ...
    sidebarSectionTitle: { fontSize: font.sizes.sectionHeader - 1, fontFamily: headerFont, fontWeight: 700 as const, color: accentColor, textTransform: 'uppercase', letterSpacing: 2, marginBottom: 6 },
    ...
    mainSectionTitle: { fontSize: font.sizes.sectionHeader + 2, fontFamily: headerFont, fontWeight: 700 as const, color: accentColor, textTransform: 'uppercase', letterSpacing: 2, marginBottom: 8 },
    ...
    projectBlock: { marginBottom: layout.itemSpacing + 4, borderLeftWidth: 3, borderLeftColor: accentColor, paddingLeft: 12 },
    projectTitle: { fontSize: font.sizes.normal + 1, fontFamily: bodyFont, fontWeight: 700 as const, color: colors.text },
    projectTech: { fontSize: font.sizes.small, fontFamily: bodyFont, fontStyle: 'italic' as const, color: accentColor, marginTop: 2 },
  ...
  const contactItems = [data.contact.email, data.contact.phone, data.contact.location, data.contact.website, data.contact.linkedin, data.contact.github, data.contact.portfolio].filter(Boolean);
  ...
  const renderMainSection = (section: SectionConfig) => {
    switch (section.type) {
      ...
      case 'projects':
        ...
        return (<View key={section.id} style={styles.mainSection}><Text style={styles.mainSectionTitle}>{section.title}</Text>{data.projects.map((entry) => (<View key={entry.id} style={styles.projectBlock}><Text style={styles.projectTitle}>{entry.name}</Text>{entry.technologies.length > 0 && <Text style={styles.projectTech}>{entry.technologies.join(' / ')}</Text>}{entry.description ? <Text style={styles.description}>{entry.description}</Text> : null}{entry.highlights.length > 0 && renderBullets(entry.highlights)}</View>))}</View>);
      case 'certifications':
        if (data.certifications.length === 0) return null;
        return (<View key={section.id} style={styles.mainSection}><Text style={styles.mainSectionTitle}>{section.title}</Text>{data.certifications.map((entry) => (<View key={entry.id} style={styles.entryBlock}><View style={styles.entryRow}><Text style={styles.entryTitle}>{entry.name}</Text><Text style={styles.entryDate}>{entry.date}</Text></View><Text style={styles.entrySubtitle}>{entry.issuer}</Text>{entry.credentialId ? <Text style={styles.description}>Credential ID: {entry.credentialId}</Text> : null}</View>))}</View>);
      case 'volunteer':
