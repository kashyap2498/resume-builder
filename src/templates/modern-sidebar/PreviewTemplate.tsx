// =============================================================================
// Modern Sidebar - Preview Template (HTML/CSS)
// =============================================================================
// Modern two-column design. Left sidebar (35%) with contact, skills, languages,
// certifications. Main area for experience, education, etc.
...
const ModernSidebarPreview: React.FC<TemplateProps> = ({ resume }) => {
    ...
    switch (section.type) {
      case 'contact':
        return (
          <div key={section.id} style={{ marginBottom: `${layout.sectionSpacing}px` }}>
            ...
            {data.contact.title && (
              <div style={{ fontSize: `${font.sizes.title}px`, fontFamily: font.family, color: 'rgba(255,255,255,0.85)', marginTop: '4px' }}>
                {data.contact.title}
              </div>
            )}
            <div style={{ marginTop: '12px' }}>
              <ContactLine
                ...
                font={{ ...font, sizes: { ...font.sizes } }}
                colors={{ ...colors, lightText: 'rgba(255,255,255,0.75)' }}
                layout="block"
              />
            </div>
          </div>
        ...
        return (
          <div key={section.id} style={{ marginBottom: `${layout.sectionSpacing}px` }}>
            ...
            {data.languages.map((lang) => (
              <div key={lang.id} style={{ fontSize: `${font.sizes.small}px`, fontFamily: font.family, color: 'rgba(255,255,255,0.85)', marginBottom: '3px' }}>
                {lang.name} - <span style={{ color: 'rgba(255,255,255,0.6)', fontStyle: 'italic' }}>{lang.proficiency}</span>
              </div>
            ))}
          </div>
        );
          ...
          <div key={section.id} style={{ marginBottom: `${layout.sectionSpacing}px` }}>
            ...
            {data.certifications.map((entry) => (
              <div key={entry.id} style={{ marginBottom: '6px' }}>
                <div style={{ fontWeight: 600, fontSize: `${font.sizes.small}px`, fontFamily: font.family, color: '#ffffff' }}>{entry.name}</div>
                <div style={{ fontSize: `${font.sizes.small - 1}px`, fontFamily: font.family, color: 'rgba(255,255,255,0.65)' }}>{entry.issuer}</div>
      ...
      case 'hobbies':
        ...
        return (
          <div key={section.id} style={{ marginBottom: `${layout.sectionSpacing}px` }}>
            <h2 style={sidebarTitleStyle}>{section.title}</h2>
            <div style={{ fontSize: `${font.sizes.small}px`, fontFamily: font.family, color: 'rgba(255,255,255,0.8)' }}>
    ...
    >
      ...
      <div
        style={{
          ...
          padding: `${layout.margins.top}px ${layout.margins.left * 0.8}px ${layout.margins.bottom}px ${layout.margins.left}px`,
          flexShrink: 0,
        }}
      >
      ...
      <div
        style={{
          width: '65%',
          padding: `${layout.margins.top}px ${layout.margins.right}px ${layout.margins.bottom}px ${layout.margins.left * 0.8}px`,
        }}
      >
...
export default ModernSidebarPreview;
