    ...
    switch (section.type) {
      ...
      case 'skills':
        if (data.skills.length === 0) return null;
        {
          ...
          const sidebarColors = { ...colors, accent: accentColor };
          return (
            <div key={section.id} style={{ marginBottom: `${layout.sectionSpacing}px` }}>
              <h2 style={sidebarTitleStyle}>{section.title}</h2>
              <SkillsBlock skills={data.skills} layout={data.skillsLayout} mode={data.skillsMode} font={sidebarFont} colors={sidebarColors} categoryColor={accentColor} />
            </div>
          );
        }
      case 'languages':
