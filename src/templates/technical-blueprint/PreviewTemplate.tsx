// =============================================================================
// Technical Blueprint - Preview Template (HTML/CSS)
// =============================================================================
// Clean technical design. Monospace-inspired headings, steel blue color palette.
...
const TechnicalBlueprintPreview: React.FC<TemplateProps> = ({ resume }) => {
    ...
    switch (section.type) {
      case 'contact':
        return (
          <div key={section.id} style={{ marginBottom: `${layout.sectionSpacing}px` }}>
            <div style={{ borderLeft: `4px solid ${steelBlue}`, paddingLeft: '12px', marginBottom: '8px' }}>
              <h1
                style={{
              ...
              {data.contact.title && (
                <div style={{ fontSize: `${font.sizes.title}px`, fontFamily: font.family, color: steelBlue, marginTop: '2px' }}>
                  {data.contact.title}
                </div>
              )}
            </div>
      ...
      case 'skills':
        ...
        return (
          <div key={section.id} style={{ marginBottom: `${layout.sectionSpacing}px` }}>
            <h2 style={sectionTitleStyle}>{section.title}</h2>
            <SkillsBlock skills={data.skills} layout={data.skillsLayout} mode={data.skillsMode} font={font} colors={colors} separator=" | " categoryWeight={700} categoryColor={steelBlue} />
          </div>
        );
          ...
          <div key={section.id} style={{ marginBottom: `${layout.sectionSpacing}px` }}>
            ...
            {data.references.map((entry) => (
              <div key={entry.id} style={{ marginBottom: `${layout.itemSpacing}px` }}>
                ...
                <div style={{ fontSize: `${font.sizes.small}px`, fontFamily: font.family, color: colors.lightText }}>{entry.title}{entry.company ? `, ${entry.company}` : ''}</div>
...
export default TechnicalBlueprintPreview;
