// =============================================================================
// Clinical Professional - Preview Template (HTML/CSS)
// =============================================================================
// Healthcare professional template. Clean, trustworthy blue palette.
...
const ClinicalProfessionalPreview: React.FC<TemplateProps> = ({ resume }) => {
  ...
  const sectionTitleStyle: React.CSSProperties = {
    ...
    letterSpacing: '1px',
    borderBottom: `2px solid ${medBlue}`,
    paddingBottom: '3px',
  };
    ...
    switch (section.type) {
      case 'contact':
        return (
          <div key={section.id} style={{ marginBottom: `${layout.sectionSpacing}px` }}>
            <div style={{ backgroundColor: medBlue, margin: `-${layout.margins.top}px -${layout.margins.right}px 0 -${layout.margins.left}px`, padding: `${layout.margins.top}px ${layout.margins.right}px 16px ${layout.margins.left}px` }}>
              <h1 style={{ fontSize: `${font.sizes.name}px`, fontFamily: font.headerFamily, color: '#ffffff', fontWeight: 700, margin: 0, lineHeight: font.lineHeight }}>
                {data.contact.firstName} {data.contact.lastName}
              </h1>
              {data.contact.title && (
                <div style={{ fontSize: `${font.sizes.title}px`, fontFamily: font.family, color: 'rgba(255,255,255,0.9)', marginTop: '4px' }}>
                  {data.contact.title}
                </div>
              )}
            </div>
      ...
      case 'summary':
        ...
        return (
          <div key={section.id} style={{ marginBottom: `${layout.sectionSpacing}px` }}>
            ...
            <p style={{ fontSize: `${font.sizes.normal}px`, fontFamily: font.family, color: colors.text, lineHeight: font.lineHeight, margin: '4px 0 0 0' }}>
              {data.summary.text}
            </p>
          </div>
        );
          ...
          <div key={section.id} style={{ marginBottom: `${layout.sectionSpacing}px` }}>
            ...
            {data.skills.map((category) => (
              <div key={category.id} style={{ marginBottom: '6px' }}>
                {category.category ? <span style={{ fontWeight: 700, fontSize: `${font.sizes.normal}px`, fontFamily: font.family, color: medBlue }}>{category.category}: </span> : null}
      ...
      case 'languages':
        ...
        return (
          <div key={section.id} style={{ marginBottom: `${layout.sectionSpacing}px` }}>
            <h2 style={sectionTitleStyle}>{section.title}</h2>
            <div style={{ fontSize: `${font.sizes.normal}px`, fontFamily: font.family, color: colors.text }}>{data.languages.map((lang) => `${lang.name} (${lang.proficiency})`).join(', ')}</div>
          </div>
        );
      ...
      case 'volunteer':
        ...
        return (
          <div key={section.id} style={{ marginBottom: `${layout.sectionSpacing}px` }}>
            <h2 style={sectionTitleStyle}>{section.title}</h2>
            {data.volunteer.map((entry) => (<EntryBlock key={entry.id} title={entry.role} subtitle={entry.organization} dateRange={formatDateRange(entry.startDate, entry.endDate)} description={entry.description} highlights={entry.highlights} font={font} colors={colors} spacing={layout.itemSpacing} />))}
          </div>
        );
      ...
      case 'awards':
        ...
        return (
          <div key={section.id} style={{ marginBottom: `${layout.sectionSpacing}px` }}>
            <h2 style={sectionTitleStyle}>{section.title}</h2>
            {data.awards.map((entry) => (<EntryBlock key={entry.id} title={entry.title} subtitle={entry.issuer} dateRange={entry.date} description={entry.description} font={font} colors={colors} spacing={layout.itemSpacing} />))}
          </div>
        );
      ...
      case 'publications':
        ...
        return (
          <div key={section.id} style={{ marginBottom: `${layout.sectionSpacing}px` }}>
            <h2 style={sectionTitleStyle}>{section.title}</h2>
            {data.publications.map((entry) => (<EntryBlock key={entry.id} title={entry.title} subtitle={entry.publisher} dateRange={entry.date} description={entry.description} font={font} colors={colors} spacing={layout.itemSpacing} />))}
          </div>
        );
      ...
      case 'affiliations':
        ...
        return (
          <div key={section.id} style={{ marginBottom: `${layout.sectionSpacing}px` }}>
            <h2 style={sectionTitleStyle}>{section.title}</h2>
            {data.affiliations.map((entry) => (<EntryBlock key={entry.id} title={entry.organization} subtitle={entry.role} dateRange={formatDateRange(entry.startDate, entry.endDate)} font={font} colors={colors} spacing={layout.itemSpacing} />))}
          </div>
        );
      ...
      case 'courses':
        ...
        return (
          <div key={section.id} style={{ marginBottom: `${layout.sectionSpacing}px` }}>
            <h2 style={sectionTitleStyle}>{section.title}</h2>
            {data.courses.map((entry) => (<EntryBlock key={entry.id} title={entry.name} subtitle={entry.institution} dateRange={entry.completionDate} description={entry.description} font={font} colors={colors} spacing={layout.itemSpacing} />))}
          </div>
        );
          ...
          <React.Fragment key={section.id}>
            {data.customSections.map((cs) => (
              <div key={cs.id} style={{ marginBottom: `${layout.sectionSpacing}px` }}>
                ...
                {cs.entries.map((entry) => (<EntryBlock key={entry.id} title={entry.title} subtitle={entry.subtitle} dateRange={entry.date} description={entry.description} highlights={entry.highlights} font={font} colors={colors} spacing={layout.itemSpacing} />))}
              </div>
            ))}
          </React.Fragment>
        );
...
export default ClinicalProfessionalPreview;
