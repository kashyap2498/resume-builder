// =============================================================================
// Professional Classic - PDF Template (@react-pdf/renderer)
// =============================================================================
...
const ProfessionalClassicPdf: React.FC<TemplateProps> = ({ resume }) => {
    ...
    switch (section.type) {
      ...
      case 'skills':
        ...
        return (
          <View key={section.id} style={styles.sectionContainer}>
            <Text style={styles.sectionTitle}>{section.title}</Text>
            <SkillsBlockPdf skills={data.skills} layout={data.skillsLayout} mode={data.skillsMode} fontSize={font.sizes.normal} fontFamily={bodyFont} textColor={colors.text} categoryColor={colors.primary} categoryFontFamily={bodyFont} />
          </View>
        );
  ...
  // Separate the contact section (rendered as full-width header) from body sections
  const contactSection = visibleSections.find((s) => s.type === 'contact');
  const bodySections = visibleSections.filter((s) => s.type !== 'contact');
