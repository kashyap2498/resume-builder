// =============================================================================
// Healthcare Modern - Preview Template (HTML/CSS)
// =============================================================================
// Modern healthcare layout with sidebar for certifications, skills, languages.
...
const HealthcareModernPreview: React.FC<TemplateProps> = ({ resume }) => {
  ...
  const sidebarTitleStyle: React.CSSProperties = {
    fontSize: `${font.sizes.sectionHeader - 1}px`, fontFamily: font.headerFamily, color: '#ffffff', fontWeight: 600,
    margin: 0, marginBottom: '8px', lineHeight: font.lineHeight, textTransform: 'uppercase', letterSpacing: '1px',
    borderBottom: '1px solid rgba(255,255,255,0.3)', paddingBottom: '4px',
  };

  const mainTitleStyle: React.CSSProperties = {
    fontSize: `${font.sizes.sectionHeader}px`, fontFamily: font.headerFamily, color: tealColor, fontWeight: 600,
    margin: 0, marginBottom: '6px', lineHeight: font.lineHeight, textTransform: 'uppercase', letterSpacing: '1px',
    borderBottom: `2px solid ${tealColor}`, paddingBottom: '4px',
  };
    ...
    switch (section.type) {
      case 'contact':
        return (
          <div key={section.id} style={{ marginBottom: `${layout.sectionSpacing}px` }}>
            ...
            {data.contact.title && <div style={{ fontSize: `${font.sizes.title}px`, fontFamily: font.family, color: 'rgba(255,255,255,0.85)', marginTop: '4px' }}>{data.contact.title}</div>}
            <div style={{ marginTop: '12px' }}>
              <ContactLine contact={data.contact} font={font} colors={{ ...colors, lightText: 'rgba(255,255,255,0.75)' }} layout="block" />
            </div>
          </div>
        );
      case 'skills':
          ...
          <div key={section.id} style={{ marginBottom: `${layout.sectionSpacing}px` }}>
            ...
            {data.skills.map((category) => (
              <div key={category.id} style={{ marginBottom: '8px' }}>
                {category.category ? <div style={{ fontWeight: 600, fontSize: `${font.sizes.small}px`, color: 'rgba(255,255,255,0.9)', marginBottom: '2px' }}>{category.category}</div> : null}
          ...
          <div key={section.id} style={{ marginBottom: `${layout.sectionSpacing}px` }}>
            ...
            {data.certifications.map((entry) => (
              <div key={entry.id} style={{ marginBottom: '6px' }}>
                <div style={{ fontWeight: 600, fontSize: `${font.sizes.small}px`, color: '#ffffff' }}>{entry.name}</div>
                <div style={{ fontSize: `${font.sizes.small - 1}px`, color: 'rgba(255,255,255,0.65)' }}>{entry.issuer}</div>
                {entry.date && <div style={{ fontSize: `${font.sizes.small - 1}px`, color: 'rgba(255,255,255,0.5)' }}>{entry.date}</div>}
              </div>
            ))}
          </div>
        );
        ...
        return (
          <div key={section.id} style={{ marginBottom: `${layout.sectionSpacing}px` }}>
            ...
            {data.languages.map((lang) => (
              <div key={lang.id} style={{ fontSize: `${font.sizes.small}px`, color: 'rgba(255,255,255,0.85)', marginBottom: '3px' }}>
                {lang.name} - <span style={{ fontStyle: 'italic', color: 'rgba(255,255,255,0.6)' }}>{lang.proficiency}</span>
              </div>
            ))}
          </div>
        );
      case 'hobbies':
        ...
        return (
          <div key={section.id} style={{ marginBottom: `${layout.sectionSpacing}px` }}>
            <h2 style={sidebarTitleStyle}>{section.title}</h2>
            <div style={{ fontSize: `${font.sizes.small}px`, color: 'rgba(255,255,255,0.8)' }}>{data.hobbies.items.join(', ')}</div>
          </div>
        );
      default:
        return null;
  ...
  return (
    <div style={{ width: '100%', maxWidth: '816px', margin: '0 auto', display: 'flex', backgroundColor: colors.background, fontFamily: font.family, color: colors.text, lineHeight: font.lineHeight, minHeight: '1056px' }}>
      <div style={{ width: '35%', backgroundColor: tealColor, padding: `${layout.margins.top}px ${layout.margins.left * 0.8}px ${layout.margins.bottom}px ${layout.margins.left}px`, flexShrink: 0 }}>
        {sidebarSections.map(renderSidebarSection)}
      </div>
      <div style={{ width: '65%', padding: `${layout.margins.top}px ${layout.margins.right}px ${layout.margins.bottom}px ${layout.margins.left * 0.8}px` }}>
...
export default HealthcareModernPreview;
